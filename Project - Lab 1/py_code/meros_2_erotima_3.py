...
# Spectrogram
img = librosa.display.specshow(S_db, sr=fs, hop_length=hop_length, x_axis='time', y_axis='hz', ax=ax[0], cmap='magma')
ax[0].set_title(f'Φασματογράφημα (Λογαριθμική Κλίμακα, Παράθυρο={win_len_ms}ms, N={n_fft})')
fig.colorbar(img, ax=ax[0], format="%+2.0f dB")

# Spectral Centroid
ax[1].plot(times, centroid, color='cyan', linewidth=2)
ax[1].set_title('Φασματικό Κέντρο (Spectral Centroid)')
ax[1].set_ylabel('Συχνότητα (Hz)')
ax[1].grid(True, linestyle='--', alpha=0.7)

# Spectral Flux
ax[2].plot(times, flux, color='lime', linewidth=2)
ax[2].set_title('Φασματική Ροή (Spectral Flux)')
